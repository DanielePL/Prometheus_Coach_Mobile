...
class ClientRepository @Inject constructor(
     ...
     * Uses the first 8 characters of the coach's UUID (uppercase).
     * This matches the mobile app's connectWithCoachCode logic.
     *
     * Example: Coach ID "faba7636-1234-..." → Code "FABA7636"
     */
    suspend fun getCoachInviteCode(): Result<String> {
        return try {
            ...
            // Use first 8 characters of UUID as the invite code
            // This matches the mobile app's lookup logic
            val code = coachId.take(8).uppercase()
            Log.d("ClientRepository", "Coach invite code: $code (from ID: $coachId)")

            Result.success(code)
        } catch (e: Exception) {
            Log.e("ClientRepository", "Failed to get invite code", e)
            Result.failure(e)
        }
    }
}
